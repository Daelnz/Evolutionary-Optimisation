function varargout = GeneticAlgorithm3Parents(varargin)
% GENETICALGORITHM3PARENTS MATLAB code for GeneticAlgorithm3Parents.fig
%      GENETICALGORITHM3PARENTS, by itself, creates a new GENETICALGORITHM3PARENTS or raises the existing
%      singleton*.
%
%      H = GENETICALGORITHM3PARENTS returns the handle to a new GENETICALGORITHM3PARENTS or the handle to
%      the existing singleton*.
%
%      GENETICALGORITHM3PARENTS('CALLBACK',hObject,eventData,handles,...) calls the local
%      function named CALLBACK in GENETICALGORITHM3PARENTS.M with the given input arguments.
%
%      GENETICALGORITHM3PARENTS('Property','Value',...) creates a new GENETICALGORITHM3PARENTS or raises the
%      existing singleton*.  Starting from the left, property value pairs are
%      applied to the GUI before GeneticAlgorithm3Parents_OpeningFcn gets called.  An
%      unrecognized property name or invalid value makes property application
%      stop.  All inputs are passed to GeneticAlgorithm3Parents_OpeningFcn via varargin.
%
...
% Edit the above text to modify the response to help GeneticAlgorithm3Parents

% Last Modified by GUIDE v2.5 26-Nov-2015 02:25:25

% Begin initialization code - DO NOT EDIT
gui_Singleton = 1;
gui_State = struct('gui_Name',       mfilename, ...
                   'gui_Singleton',  gui_Singleton, ...
                   'gui_OpeningFcn', @GeneticAlgorithm3Parents_OpeningFcn, ...
                   'gui_OutputFcn',  @GeneticAlgorithm3Parents_OutputFcn, ...
                   'gui_LayoutFcn',  [] , ...
                   'gui_Callback',   []);
...
% --- Executes just before GeneticAlgorithm3Parents is made visible.
function GeneticAlgorithm3Parents_OpeningFcn(hObject, eventdata, handles, varargin)
% This function has no output args, see OutputFcn.
% hObject    handle to figure
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
% varargin   command line arguments to GeneticAlgorithm3Parents (see VARARGIN)

% Choose default command line output for GeneticAlgorithm3Parents
handles.output = hObject;

% Update handles structure
guidata(hObject, handles);

% UIWAIT makes GeneticAlgorithm3Parents wait for user response (see UIRESUME)
% uiwait(handles.figure1);


% --- Outputs from this function are returned to the command line.
function varargout = GeneticAlgorithm3Parents_OutputFcn(~, eventdata, handles) 
...
NumGenes = str2double(get(handles.NSize, 'String'));
...
for j = 1:Population
    for i = 1:NumGenes    
        % r = a+ (b-a)*rand
        Individual(j).Gene(i) = round((-5.12 + (5.12+5.12)*rand),2);
    end
end
...
dlmwrite('Function3ParentsResults.csv', [XAxisData; YAxisData],'-append', 'delimiter', '\t', 'newline', 'pc')  
...
for k = 1:Population
    ...
    for i = (1:NumGenes)
        TempFit = TempFit + (Individual(k).Gene(i)^2) - (10*cos(2*pi*Individual(k).Gene(i)));
    end
    TempFit = 10*NumGenes + TempFit;
    
    % For displaying actual fitness value at the end
...
for k = 1:Population
    ...
    for n = 1:Population
       if SelectionWheel(n) >= Selection
        ...
        % Apply Crossover
        CrossOverPoint1 = round(rand*(NumGenes-1))+1; 
        CrossOverPoint2 = round(rand*(NumGenes-1))+1; 
        ...
        % Make sure CrossOverPoint1 holds the lesser value
        if CrossOverPoint2 < CrossOverPoint1
            TempCross = CrossOverPoint1;
            CrossOverPoint1 = CrossOverPoint2;
            CrossOverPoint2 = TempCross;            
        end
        
        % Offspring based on highest fitness parent + probability of crossover 
        if (Individual(Parent1).Fitness >= Individual(Parent2).Fitness) && (Individual(Parent1).Fitness >= Individual(Parent3).Fitness)
            Offspring(k).Gene = Individual(Parent1).Gene;
            if rand < CrossOverRate
                Offspring(k).Gene(CrossOverPoint1:CrossOverPoint2) = Individual(Parent2).Gene(CrossOverPoint1:CrossOverPoint2);
                Offspring(k).Gene(CrossOverPoint2:end) = Individual(Parent3).Gene(CrossOverPoint2:end);
                
            end
        elseif (Individual(Parent2).Fitness >= Individual(Parent1).Fitness) && (Individual(Parent2).Fitness >= Individual(Parent3).Fitness)
            Offspring(k).Gene = Individual(Parent2).Gene;
            if rand < CrossOverRate
                Offspring(k).Gene(CrossOverPoint1:CrossOverPoint2) = Individual(Parent1).Gene(CrossOverPoint1:CrossOverPoint2);
                Offspring(k).Gene(CrossOverPoint2:end) = Individual(Parent3).Gene(CrossOverPoint2:end);      
            end       
        else
            Offspring(k).Gene = Individual(Parent3).Gene;
            if rand < CrossOverRate
                Offspring(k).Gene(CrossOverPoint1:CrossOverPoint2) = Individual(Parent1).Gene(CrossOverPoint1:CrossOverPoint2);
                Offspring(k).Gene(CrossOverPoint2:end) = Individual(Parent2).Gene(CrossOverPoint2:end);      
            end   
        end
        ...
        for l = 1:NumGenes
             if rand < MutationRate
                % Add x to gene where -2 <= x <= 2 
                Offspring(k).Gene(l) = Offspring(k).Gene(l) + round((-2 + (2+2)*rand),2);
                % Set gene to nearest limit if it exceeds it
                if Offspring(k).Gene(l) > 5.12
                    Offspring(k).Gene(l) = 5.12;
                elseif Offspring(k).Gene(l) < -5.12
                    Offspring(k).Gene(l) = -5.12;
                end  
             end
        end
